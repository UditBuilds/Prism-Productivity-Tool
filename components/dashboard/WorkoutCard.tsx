      ...
      <div className="rounded-xl border border-border bg-surface px-4 py-3.5">
        ...
        <div className="mt-3.5">
          {isLoading ? (
            ...
            <p className="py-1 text-[13px] text-muted-foreground">
              Nothing logged today — type a set above and it saves straight
              away.
            </p>
          ) : (
        ...
        {/* The feature measuring its own use. Hidden until there is something
            to report, so a brand-new user isn't greeted by a zero. */}
        {(sessionCount ?? 0) > 0 && (
...
function SetEditor({ set, onClose }: { set: WorkoutSet; onClose: () => void }) {
  ...
  function save() {
    updateSet.mutate({
      ...
      reps: (() => {
        const n = numberOrNull(reps, set.reps);
